...
let rec prepare_pre_post_context_exp e encgamma pre post = 
 match e with
  | EVar v -> (VarLocMap.add (Reg v) (VarLocMap.find (Reg v) encgamma) pre, post)
  | EIsunset l 
  | ELoc l -> (VarLocMap.add (Mem l) (VarLocMap.find (Mem l) encgamma) pre, post)
  | EConstant n -> (pre, post)
  | ELam(mu, pre, kpre, p, u, post, kpost, q, s) -> (pre, post)
  | EPlus (l,r) 
  | EModulo (l,r)
  | EEq (l,r) 
  | ENeq (l,r) -> let (pre1, post1) = prepare_pre_post_context_exp l encgamma pre post in
		prepare_pre_post_context_exp r encgamma pre1 post1
  | ETrue 
  | EFalse ->  (pre, post)
  | EDeref e ->  prepare_pre_post_context_exp e encgamma pre post

let rec prepare_pre_post_context s encgamma pre post = 
 match s with
   EIf (e,s1,s2) -> let (pre', post') =  prepare_pre_post_context_exp e encgamma pre post in
		    let (encgamma1, pre1, post1) = prepare_pre_post_context s1 encgamma pre' post' in
			prepare_pre_post_context s2 encgamma1 pre1 post1 
  |ESkip  	-> (encgamma, pre, post)
  |EDeclassify (x, e)  
  |EAssign (x,e) -> 
		let (pre', post') =  prepare_pre_post_context_exp e encgamma pre post in
  ...
  |EUpdate (e1, e2) ->
		let (pre1, post1) =  prepare_pre_post_context_exp e1 encgamma pre post in
		let (pre2, post2) =  prepare_pre_post_context_exp e2 encgamma pre1 post1 in
		(encgamma, pre2, post2)
  |EESeq eli  -> let rec loop eli encgamma pre post = begin match eli with
		 |[] -> (encgamma, pre, post)
		 |xs::tail -> let (encgamma', pre', post') = prepare_pre_post_context xs encgamma pre post in
  ...
  |EWhile (e,s) -> 
		let (pre', post') =  prepare_pre_post_context_exp e encgamma pre post in
		prepare_pre_post_context s encgamma pre' post'

  |EOutput (_, e) -> 
		let (pre', post') =  prepare_pre_post_context_exp e encgamma pre post in
		(encgamma, pre', post')
  |ECall f -> 
		let (pre', post') =  prepare_pre_post_context_exp f encgamma pre post in
		let encgamma' = enc_flow_sensitive_type_infer Low encgamma s in
		(encgamma', pre', post')
  |ESet l  -> 
		let (pre1, post1) =  prepare_pre_post_context_exp (ELoc l) encgamma pre post in
		(encgamma, pre1, post1)
...
(* [next_fvar ()] generates a fresh type variable *)
let next_fvar () : var =
  let x = !fvar_cell in
  let s = "f" ^ string_of_int x in
  incr fvar_cell;  s
